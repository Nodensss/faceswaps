...
- Экран во время обработки не удерживается от засыпания (§9.4) — относится к
  оставшейся части E2.
- Пункт ручной приёмки §14.4 п. 8 (пять циклов без роста heap), обработка 12 МП без
  OOM и поведение при thermal throttling не выполнялись: §15 явно откладывает эти три
  пункта DoD этапа E2 до физического ARM reference device.
- Пункты ручной приёмки §14.4 пп. 1–7 на реальном фото не выполнялись: этой
  контрольной точкой закрыт только сценарий сохранения на синтетическом debug-harness.
  Полная приёмка пп. 1–7 относится к закрытию всего этапа E2.
