...
- **`android:largeHeap="true"` в манифесте.** Это не косметика: `dalvik.vm.heapgrowthlimit`
  ...
  `Runtime.maxMemory()` = 512 МиБ и бюджет на этом AVD составляет около 20 млн px
  (точное значение зависит от занятости кучи в момент декодирования; замеренные
  прогоны дали 20 039 538 и 20 039 691 px). Бюджет по-прежнему читает фактический
  потолок, то есть ограничение не отменено, а поднято.
