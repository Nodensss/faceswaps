...
## E2, контрольная точка 2: полное разрешение 12 МП, API 35 x86_64

Дата: 04.08.2026. AVD `QuizMaster_API35`, Android 15 / API 35, x86_64, 2 виртуальных
ядра, `MemTotal: 2019876 kB`, ONNX Runtime Android 1.26.0, CPU, `airplane_mode_on=1`.
Тест: `StageE2FullResolutionPipelineInstrumentedTest`, полный прогон 104,307 с,
`OK (1 test)`.

Фикстура — закоммиченный `stage_d_group_target.png`, вписанный без искажения пропорций
в холст 4000×3000 (12 МП) и сохранённый как JPEG q95. Одно назначение, восстановление
GFPGAN силой `0.8`. Экспорт выполнен **настройками по умолчанию** (JPEG q95, видимый
водяной знак включён), потому что знак копирует весь bitmap результата и измерение без
него занизило бы реальный пик. Замеры сняты семплером внутри процесса приложения
(355 семплов, интервал 250 мс; PSS каждый восьмой семпл).

| Величина | Значение |
| --- | ---: |
| Размер цели | 4000×3000 (12 000 000 px) |
| Бюджет `ImageMemoryBudget` в момент декодирования | 20 039 538 px |
| Декодировано в полном разрешении | да |
| Время координатора (свап + GFPGAN, одно лицо) | 87 346 ms |
...
| Изменено пикселей вне union(swap, enhance) ROI | 0 |
| Размер экспортированного файла | 4000×3000 |

Выводы:

1. **2-гигабайтный AVD вытянул 12 МП.** Пиковый суммарный PSS 1 294,3 МиБ укладывается
   в ориентир §12 (~1,5 ГБ), но занимает около 86% от него; на устройстве с меньшим
   лимитом кучи адаптивный бюджет сам уменьшит цель, а UI сообщит об этом перед
   сохранением.
2. **`largeHeap` обязателен, а не косметичен.** Пиковая Java-куча 161,6 МиБ измерена
   против потолка 512 МиБ. Значение `dalvik.vm.heapgrowthlimit` этого AVD — `192m`;
   без `android:largeHeap="true"` живых данных было бы почти вровень с потолком, а
   адаптивный бюджет при 192 МиБ ограничил бы цель примерно 5 МП вместо 20 МП.
3. **Native heap доминирует.** 1 249,1 МиБ приходится в основном на арены ONNX Runtime
   (GFPGAN, InSwapper, BiSeNet) и на bitmap-пиксели, которые с API 26 живут вне
   Java-кучи. Полнокадровые `IntArray`/`FloatArray` компоновщика — это Java-часть.
4. **Водяной знак стоит один полный кадр.** Тот же сценарий с выключенным знаком дал
   peak native 1 191,6 МиБ и peak PSS 1 244,1 МиБ; включённый по умолчанию знак
   добавляет около 57,5 МиБ native, потому что рисуется в копии результата и не
   изменяет bitmap, который пользователь сравнивает в режиме «До/После».
5. Пространственный инвариант сохраняется на 12 МП: вне объединения paste ROI изменено
   ровно 0 пикселей из 12 000 000.

Замеренные значения относятся только к этому x86_64 AVD и не являются прогнозом для
ARM-телефона: reference device по-прежнему не подключён (§12). Пять повторных циклов
без устойчивого роста heap и поведение при thermal throttling здесь не проверялись —
эти пункты DoD этапа E2 остаются отложенными до физического устройства.

Команды:

```powershell
adb shell am instrument -w -r -e class com.faceswaplocal.app.inference.StageE2FullResolutionPipelineInstrumentedTest com.faceswaplocal.app.test/androidx.test.runner.AndroidJUnitRunner
adb shell run-as com.faceswaplocal.app cat files/stage_e2_full_resolution.json
adb shell getprop dalvik.vm.heapgrowthlimit
adb shell getprop dalvik.vm.heapsize
```

## E1, контрольная точка 3: UI качества и диагностика цвета, API 35 x86_64

Дата: 2026-08-02. AVD `QuizMaster_API35`, Android 15 / API 35, x86_64,
ONNX Runtime Android 1.26.0, CPU, `airplane_mode_on=1`. Compose instrumentation
настройки/сравнения после финальной переустановки APK: 208,019 с, `OK (3 tests)`
(предыдущий прогон того же класса — 77,063 с; AVD был замедлен после inference).
Диагностический instrumentation
`pair_02`/`pair_03`: 321,736 с, `OK (1 test)`; внутри него GFPGAN inference занял
46 941 и 62 051 ms соответственно.

Production color matching считает RGB mean/std по аффинной box-маске выровненного
кропа 128×128, а не по BiSeNet parser-region: threshold `0.5` выбрал 12 024 из
16 384 samples, суммарный вес — 11 254,042074. Взвешенное пересечение этой области
с parser-mask составило только 53,952865% для `pair_02` и 53,880621% для `pair_03`;
следовательно, в статистику действительно входят волосы и фон вне parser-области.

Тон измерен по заранее зафиксированным polygon ROI синтетических fixtures в
`sRGB → CIE Lab D65`; основная величина — CIEDE2000 между медианами face и neck.
`Residual` — ошибка вектора `face − neck` относительно исходного target в Lab/ΔE76,
поэтому учитывает, что на `pair_03` исходное освещение само создаёт большой разрыв.

| Пара | Target face↔neck ΔE00 | После swap + color ΔE00 | После GFPGAN 0.8 ΔE00 | Residual после swap | Residual после GFPGAN | GFPGAN drift |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
GFPGAN улучшил соответствие исходному отношению face↔neck на `pair_02`, но ухудшил
его на `pair_03`; neck ROI в обоих кадрах и на обоих этапах изменил ровно 0 пикселей.
Диагностика не меняет production-коэффициенты: fixtures синтетические, а результат
не даёт основания включать сохранение цветности/только детализацию яркости без
перепроверки на разрешённом реальном фото и физическом reference device. Полные Lab,
RGB, mask-contract и PNG:
`docs/parity/android/api35-x86_64/checkpoint_3/`.

Пиковые пары тяжёлых сессий не изменились относительно контрольной точки 2:
BiSeNet+InSwapper в первом проходе, BiSeNet+GFPGAN во втором. Peak Java/native heap
и thermal status не измерялись; AVD не является reference device.
