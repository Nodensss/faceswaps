...
T1←source 1, T2←source 2, T3←source 3, T4=`Не менять`. На Android выполнены три
...
3. сила `1.0`: реальный worst-case прогон, без интерполяции результата `0.8`.
...
Фактический повторный прогон: AVD API 35 x86_64, ONNX Runtime Android 1.26.0, CPU,
`airplane_mode_on=1`; `OK (1 test)`, 887,171 с. Coordinator-время: 170 439 мс без
restoration, 322 886 мс при силе 0.8 и 328 106 мс при силе 1.0.
...
| Инвариант | Сила 0 | Сила 0.8 | Сила 1.0 |
| --- | ---: | ---: | ---: |
| Изменено пикселей вне `union(swap ROI, enhance ROI)` | 0 | 0 | 0 |
| Изменено пикселей в полном потенциальном `ffhq_512` ROI T4 | 0 | 0 | 0 |
| Enhance target IDs | — | T1, T2, T3 | T1, T2, T3 |
...
Три набора margin (`0`, `0.8`, `1.0`) зафиксированы как baseline AVD. При появлении
первого reference device их нужно повторить на разрешённом реальном фото тем же
ArcFace-критерием; значения AVD не заменяются, а сохраняются рядом для сравнения.
